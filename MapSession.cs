    ...
    public class MapSession : IDisposable
    {
        ...
        ~MapSession()
        {
            foreach (var map in Maps)
            {
                if (map.Controller)
                {
                    UnityEngine.Object.Destroy(map.Controller.gameObject);
                }
            ...
            {
                UnityEngine.Object.Destroy(builderMapController.gameObject);
            }
        }
        ...
        public void LoadMapMeta(
            SparseSpatialMapController mapControllerPrefab, 
            bool particleShow, 
            bool isCreateProps = true,
            Action<SparseSpatialMapController, GameObject, MapMeta.PropInfo> objectCreatedEvent = null, 
            Action<SparseSpatialMapController, SparseSpatialMapController.SparseSpatialMapInfo> mapLoadEvent = null, 
            Action<SparseSpatialMapController> mapLocalized = null, 
            Action<SparseSpatialMapController> mapStopLocalize = null)
        {
            ...
            foreach (var m in Maps)
            {
                ...
                {
                    GUIPopup.EnqueueMessage("Load map {name = " + map.Name + ", id = " + map.ID + "} into " +
                                            MapWorker.name + Environment.NewLine +
                    ...
                    GUIPopup.EnqueueMessage("Notice: By default (MainViewRecycleBinClearMapCacheOnly == false)," +
                                            Environment.NewLine +
                                            "load map will not trigger a download in this sample." +
                                            Environment.NewLine +
                                            "Map cache is used (SparseSpatialMapManager.clear not called alone)." +
                                            Environment.NewLine +
                                            "Statistical request count will not be increased (more details on EasyAR website).",
                        5);
                    ...
                    if (isCreateProps)
                    {
                        foreach (var propInfo in meta.Props)
                        {
                            ...
                            prop.transform.localPosition = new UnityEngine.Vector3(propInfo.Position[0],
                                propInfo.Position[1], propInfo.Position[2]);
                            prop.transform.localRotation = new Quaternion(propInfo.Rotation[0], propInfo.Rotation[1],
                                propInfo.Rotation[2], propInfo.Rotation[3]);
                            prop.transform.localScale =
                                new UnityEngine.Vector3(propInfo.Scale[0], propInfo.Scale[1], propInfo.Scale[2]);
                            prop.name = propInfo.Name;

                            objectCreatedEvent?.Invoke(controller, prop, propInfo);

                            m.Props.Add(prop);
                        }
                    }

                    mapLoadEvent?.Invoke(controller, map);
                    
                    Debug.Log("MapSession MapLoad: " 
                ...
                controller.MapLocalized += () =>
                {
                    ...
                    mapLocalized?.Invoke(controller);
                    Debug.Log("MapSession MapLocalized: " 
                              + controller.transform.position + "/" 
                              + controller.transform.localPosition);
                ...
                controller.MapStopLocalize += () =>
                {
                    ...
                    mapStopLocalize?.Invoke(controller);
                };
            ...
            MapWorker.BuilderMapController.MapHost += (map, isSuccessful, error) =>
            {
                if (isSuccessful)
                {
                    ...
                    GUIPopup.EnqueueMessage("Notice: map name can be changed on website," + Environment.NewLine +
                                            "while the SDK side will not get updated until map cache cleared and a re-download has been triggered.",
                        5);
                    Saved = true;
                }
                else
                {
                    GUIPopup.EnqueueMessage(
                        "Fail to generate Map" + (string.IsNullOrEmpty(error) ? "" : "\n error: " + error), 5);
                }
