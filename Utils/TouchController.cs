...
namespace ZBoom.Common
{
    public class TouchController : MonoBehaviour
    {
        public Action<Quaternion> RotateAction; 
        public Action<Vector3> ScaleAction; 
        ...
        private void Update()
        {
            ...
            else if (curGesture == GestureControl.TwoMove || curGesture == GestureControl.TwoRotate ||
            ...
            else if (curGesture == GestureControl.TwoWait)
            {
                ...
                else
                {
                    ...
                    if (touch1Delta.magnitude > gestureEnableDistanceThreshold &&
                        touch2Delta.magnitude > gestureEnableDistanceThreshold)
                    {
                        StopAllCoroutines();
        ...
        public void TurnOn(Transform target, Camera cam, bool isOneFingerDraggable, bool isTwoFingerDraggable,
            bool isTwoFingerScalable, bool isTwoFingerRotatable)
        {
            Debug.Log("TurnOn");
            StopAllCoroutines();
        ...
        private IEnumerator OnOneMove()
        {
            ...
            while (Input.touchCount == 1 && originalPosition.ContainsKey(Input.GetTouch(0).fingerId))
            {
                ...
                var touchV3 = new Vector3(Input.GetTouch(0).deltaPosition.x / Screen.width,
                    Input.GetTouch(0).deltaPosition.y / Screen.height, 0);
        ...
        private IEnumerator OnTwoMove()
        {
                   ...
                   originalPosition.ContainsKey(Input.GetTouch(1).fingerId))
            {
                ...
                Vector2 fixedDelta = (Input.GetTouch(0).position + Input.GetTouch(1).position -
                                      originalPosition[Input.GetTouch(0).fingerId] -
                                      originalPosition[Input.GetTouch(1).fingerId]);
                ...
                if (yMovement != Vector3.zero)
                {
                    ...
                    var newPos = rawTargetPos +
                                 (Vector3.Dot(yMovement, Cam_Forward) > 0 ? Cam_Forward_XZ : -Cam_Forward_XZ) *
                                 yMovement.magnitude * targetCamDistance * 2 / 1000;
                    controlTarget.position = newPos;
                }
                   ...
                   originalPosition.ContainsKey(Input.GetTouch(1).fingerId))
            {
                ...
                Vector2 fixedDelta =
                    (Input.GetTouch(0).position + Input.GetTouch(1).position -
                     originalPosition[Input.GetTouch(0).fingerId] - originalPosition[Input.GetTouch(1).fingerId]) *
                    0.5f /
                    Screen.width * rotateSpeed;
                GetRelativeTouch(fixedDelta, out xMovement, out yMovement);
                if (xMovement != Vector3.zero)
                {
                    if (Vector3.Dot(Vector3.Cross(xMovement.normalized, Vector3.up), Camera.main.transform.forward) <
                        0f)
                    {
                        controlTarget.rotation =
                            rawRotation * Quaternion.Euler(0f, xMovement.sqrMagnitude / Mathf.PI, 0f);
                    }
                    else
                    {
                        controlTarget.rotation =
                    ...
                    RotateAction?.Invoke(controlTarget.localRotation);
                }
        ...
        private IEnumerator OnTwoScale()
        {
            ...
            float rawFingersDistance = Vector2.Distance(originalPosition[Input.GetTouch(0).fingerId],
                originalPosition[Input.GetTouch(1).fingerId]);
        ...
        {
            Debug.Log("GetRelativeTouch");
            Debug.Log("GetRelativeTouch delta = " + delta);
            ...
            if (delta != Vector2.zero)
            {
                ...
                else
                {
                    ...
                    if (temp != Vector3.zero)
                    {
                        ...
                        {
                            yMovement = temp.magnitude *
                                        ((Vector3.Dot(relaForward, temp.normalized) > 0 ? relaForward : -relaForward));
                        }
                        else
                        {
                            yMovement = temp.magnitude *
                                        ((Vector3.Dot(Vector3.up, temp.normalized) > 0 ? relaForward : -relaForward));
                        }
                    }
                    else
                    {
